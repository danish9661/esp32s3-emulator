...
RUSTFLAGS="-Cprofile-generate=$PROFDIR/raw" cargo build --release -p esp32s3-emu --example run_flash

echo "[pgo] training (hello boot)..."
LLVM_PROFILE_FILE="$PROFDIR/raw/hello-%m.profraw" \
...
ADC_INJECT_MV=825 LLVM_PROFILE_FILE="$PROFDIR/raw/periph-%m.profraw" \
...
llvm-profdata merge -o "$PROFDIR/merged.profdata" "$PROFDIR"/raw/*.profraw

echo "[pgo] optimized build..."
RUSTFLAGS="-Cprofile-use=$PROFDIR/merged.profdata" cargo build --release -p esp32s3-emu --example run_flash
